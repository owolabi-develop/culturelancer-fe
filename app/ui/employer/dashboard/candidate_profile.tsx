...
export default function EmployerCandidateProfile() {
  ...
  const { data, error, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}careerportal/employer-candidate-profile-details/${id}/`,
    fetcher
  );
  ...
  if (isLoading) {
    return (
      <div className="bg-slate-50 drop-shadow-lg rounded-md animate-pulse">
    ...
    <>
        ...
        <div className="w-full flex flex-row my-2">
          ...
          <div className="w-1/2 sm:w-10 p-2 justify-end md:w-[20%] cursor-pointer">
            ...
            <div className="[&>*]:py-1 [&>*]:px-1 [&>*]:mx-2 [&>*]:bg-gray-400 [&>*]:rounded-full my-3 [&>*]:text-sm">
              {data?.skill.slice(0, 2)?.map((options: SkillI) => (
                <span key={options.id}>{options.skill}</span>
              ))}
      ...
      <section className="md:grid grid-cols-1 p-20 w-full justify-around">
        ...
        <div className="w-full">
          ...
          <div className="my-4">
            <button className="bg-[gray] py-3 px-4 rounded">
              Message Candidate
            </button>
            <button className="border py-3 px-4 rounded mx-2">
              Invite to Apply
            </button>
          </div>
        </div>
          ...
          {data?.work_experience ? (
            <>
              {data?.work_experience?.map((exp: ExperienceI) => (
                <div
                  key={exp?.id}
                  className="bg-white drop-shadow-lg w-full rounded my-4"
                >
                  <div className="w-full border-b-2 py-4 px-4">
                    <h1>{exp?.title}</h1>
                    <p>
                      {exp?.company_name} | {exp.start_date} -{" "}
                      {exp.present === true ? "Present" : ""}
                    </p>
                  </div>
                  <div className="px-10 py-4">
                    <p className="break-all w-[70%]">{exp?.description}</p>
                  </div>
                </div>
              ))}
            </>
          ...
          {data?.skill ? (
            <>
              {data?.skill.slice(0, 4)?.map((skills: AllSkillI) => (
                <div
                  key={skills?.id}
                  className="bg-white drop-shadow-lg rounded py-4 px-4 my-8"
                >
                  {/* progress bar */}
          ...
          {data?.award_certifications ? (
            <>
              {data?.award_certifications?.map((certi: CertificateI) => (
                <div
                  key={certi?.id}
                  className="bg-white drop-shadow-lg rounded py-5 px-5 my-8"
                >
                  ...
                  <button className="py-2 border px-10 rounded">
                    View Certificate
                  </button>
                </div>
              ))}
            ...
            <p className="my-10 mx-10 text-2xl text-gray-500">
              No Certifications
            </p>
          )}
        </div>
          ...
          {data?.projects ? (
            <>
              {data?.projects?.map((proj: ProjectI) => (
                <div
                  key={proj?.id}
                  className="bg-white drop-shadow-lg rounded my-8"
                >
                  ...
                  <div className="px-5 py-4">
                    ...
                    <button className="py-1 rounded border px-4 my-3">
                      <Link href={proj?.project_links_live_demo}>
                        View Project
                      </Link>
                    </button>
                  </div>
