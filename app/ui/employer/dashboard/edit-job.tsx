...
export function EmployerEditJobs() {
 ...
 const {id} = useParams<{id:string}>();
  ...
  const paymentsTerms = ["Upfront Payment","Milestone Base Payment","Payment upon Completion"]
  ...
  //  handle paymentterms change
   ...
   /// handle job experience change
   const handleCheckboxlocationTypes = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value, checked } = e.target;
    if (checked) {
...
const { data,isLoading} =   useSWR(`${process.env.NEXT_PUBLIC_API_BASE_URL}careerportal/job/${id}/`, fetcher)
if(isLoading){
  return <>
...
<section className="md:grid grid-cols-1 p-20 w-full justify-around">
      <ToastContainer />
      <div className=" [&>*]:my-4">
        <h1 className="text-4xl font-bold w-full">Edit Job</h1>
                ...
                <div className="flex">
                  ...
                  <div key={index} className="flex items-center me-4">
                    <input
                      ...
                      defaultChecked={data?.experience_levels}
                      className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded" onChange={handleCheckboxexperience}/>
                 ...
                 {jobTypeOptions.map((options,index)=>(
                  <div key={index} className="flex items-center me-4">
                    <input
                      ...
                      defaultChecked={data?.job_type?.includes(options)}
            ...
            <div className="flex">
              ...
              <div key={index} className="flex items-center me-4">
                <input
                  ...
                  required
                  defaultChecked={data?.payments_terms?.includes(options)}
                />
              ...
              {LocationsType.map((options,index)=>(
                <div key={index} className="flex items-center me-4">
                  <input
                    ...
                    defaultChecked={data?.location_type?.includes(options)}
...
// if(isLoading){
//     return <div className='bg-slate-50 drop-shadow-lg rounded-md animate-pulse py-1 px-4 my-5'></div>
// }
