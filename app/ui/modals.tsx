...
export function EducationModal({notifytest}:{notifytest:string}) {
  ...
  const { register, reset, handleSubmit,formState: { errors } } = useForm<Inputs>({resolver:zodResolver(educationSchame)});
    // handle toast bar
    const notify = () => {
        ...
        <Modal.Body>
          <div className="space-y-6 ">
           ...
           <div className="[&>*]:my-5">
            ...
            <input color="" type="text" {...register('institution_name')} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"/>
            ...
            <input color="" type="text" {...register('degree')} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"/>
            ...
            <input color="" type="text" {...register('field_of_study')} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"/>
            ...
            <input color="" type="date" {...register('start_date')} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" />
            ...
            <input color="" type="date" {...register('end_date')} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"/>
...
export function ExperienceModal({notifytest}:{notifytest:string}) {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const { register,reset, handleSubmit,formState: { errors } } = useForm<Exp>({resolver:zodResolver(exprienceSchame)});
     // handle toast bar
     const notify = () => {
      toast.success(notifytest);
  }
        ...
        <Modal show={openModal} size="md" onClose={onCloseModal} popup>
          ...
          <Modal.Body>
           ...
           <div className="[&>*]:my-5">
            ...
            <input color="" type="text" {...register('title')} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"/>
            ...
            <input color="" type="text" {...register('company_name')} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" />
            ...
            <div>
                ...
                <select id="location" {...register('location_type')} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg ">
            ...
            <div>
            <label htmlFor="location"  className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Employment type</label>
            <select id="location" {...register('employment_types')} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg ">
...
const TechSkillEnum = z.enum([
  'Python', 'JavaScript', 'React', 'Django', 'Node.js', 'SQL','nextjs','postgres', 'Java', 'C++', 
  'Ruby', 'TypeScript', 'R', 'Scala', 'Go', 'MATLAB', 'Julia', 'Vue', 'Angular',
  'Flask', 'NoSQL', 'Apache Spark', 'Hadoop', 'Kafka', 'Airflow', 'ETL', 'BigQuery',
  'Redshift', 'Snowflake', 'MongoDB', 'Pandas', 'NumPy', 'TensorFlow', 'Keras', 
  'PyTorch', 'Scikit-Learn', 'XGBoost', 'LightGBM', 'NLTK', 'SpaCy', 'AWS', 'Azure',
  'GCP', 'Docker', 'Kubernetes', 'Terraform', 'Tableau', 'Power BI', 'Looker',
  'Data Studio', 'Matplotlib', 'Seaborn', 'Plotly', 'Git', 'Jenkins', 'Ansible', 
  'Apache Beam', 'Presto'
]);
  const [openModal, setOpenModal] = useState(false);
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const { register,reset, handleSubmit,formState: { errors } } = useForm<Skl>({resolver:zodResolver(skills)});
     // handle toast bar
     const notify = () => {
      toast.success(notifytest);
  }
...
if (response.ok){
  ...
  notify()
    ...
    return (
      <>
        <Button color=""  onClick={() => setOpenModal(true)} className="bg-[#727272]  rounded text-white mb-5 md:mb-0 ">+ Add Skills</Button>
        <Modal show={openModal} size="md" onClose={onCloseModal} popup>
          <Modal.Header />
          <Modal.Body>
            <div className="space-y-6 w-full">
